...
describe('constraint registry derivations', () => {
  it('derives the draw-tool membership sets', () => {
    ...
    expect(OUTER_CLUE_TYPES).toEqual(new Set(['x_sums', 'sandwich_sums', 'skyscrapers', 'little_killers', 'numbered_rooms', 'battlefield', 'next_to_nine']))
    expect(SINGLE_CELL_TYPES).toEqual(new Set(['odd_cells', 'even_cells', 'minimums', 'maximums', 'row_index_cells', 'col_index_cells']))
    expect(LOCAL_TOOL_TYPES.size).toBe(35)
  ...
  it('derives the theme style registry with all 39 themeable keys', () => {
