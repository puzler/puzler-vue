...
describe('constraint registry derivations', () => {
  it('derives the draw-tool membership sets', () => {
    expect(CONSTRAINT_LINE_TYPES).toEqual(new Set(['renban', 'german_whispers', 'dutch_whispers', 'palindrome', 'region_sum', 'between_lines']))
    ...
    expect(LOCAL_TOOL_TYPES.size).toBe(26)
  ...
  it('derives the theme style registry with all 30 themeable keys', () => {
  ...
  it('derives the archive filter groups with their historical option order', () => {
    ...
    expect(lines).toEqual([
      ...
      'dutch_whispers', 'palindrome', 'region_sum', 'between_lines',
    ])
