...
describe('constraint registry derivations', () => {
  it('derives the draw-tool membership sets', () => {
    expect(CONSTRAINT_LINE_TYPES).toEqual(new Set(['renban', 'german_whispers', 'dutch_whispers', 'palindrome', 'region_sum', 'entropic_lines', 'between_lines']))
    ...
    expect(LOCAL_TOOL_TYPES.size).toBe(27)
  ...
  it('derives the theme style registry with all 31 themeable keys', () => {
    ...
    expect(keys.filter(k => CONSTRAINT_STYLE_REGISTRY[k as keyof typeof CONSTRAINT_STYLE_REGISTRY].category === 'lines')).toEqual([
      ...
      'entropic_lines', 'between_lines', 'thermometer', 'slow_thermometer', 'arrow',
    ])
  })
  ...
  it('derives the archive filter groups with their historical option order', () => {
    ...
    expect(lines).toEqual([
      ...
      'dutch_whispers', 'palindrome', 'region_sum', 'entropic_lines', 'between_lines',
    ])
