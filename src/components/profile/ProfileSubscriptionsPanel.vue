...
import ProfileSubscribedSeriesDocument from '@/graphql/gql/users/queries/ProfileSubscribedSeries.graphql'
import type { ProfileSubscribedSeriesQuery } from '@/graphql/generated/types'

const props = defineProps<{ username: string }>()

type SubscribedSeries = NonNullable<ProfileSubscribedSeriesQuery['user']>['subscribedSeries']['nodes'][number]

const list = useFilterableList<ProfileSubscribedSeriesQuery, SubscribedSeries>({
  query: ProfileSubscribedSeriesDocument,
  select: (d) => d.user?.subscribedSeries,
